...
void fragment() {
	...
	if (channel == 0) {
		...
		color.r = clamp(color.r - add * 0.25, 0.0, 1.0);
		color.g = clamp(color.g - add * 0.25, 0.0, 1.0);
		color.b = clamp(color.b - add * 0.25, 0.0, 1.0);
		color.a = clamp(color.a - add * 0.25, 0.0, 1.0);
	} else {
		if (channel == 1) {
