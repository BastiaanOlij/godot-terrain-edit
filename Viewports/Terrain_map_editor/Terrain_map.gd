...
# investigate if we can extend from editor.gd at some point, need to add better logic for supporting multiple brushes

#########################################################################################
# Brush strenght

export var brush_strength = 200.0 setget set_brush_strength, get_brush_strength
...
func set_brush_pos(pos):
	...
	if $Smooth:
		$Smooth.rect_position = centered_pos
...
func do_brush(pos, multi):
	# just place all brushes, the visible one will render
	set_brush_pos(pos)
	
	# change the strenght, this can become negative
...
func do_smooth(pos, multi):
	# just place all brushes, the visible one will render
	set_brush_pos(pos)
	
	# change the strenght, this can become negative
	$Smooth.material.set_shader_param("strength", 50.0 * multi)
