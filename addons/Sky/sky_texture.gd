...
func set_time_of_day(hours, directional_light, horizontal_angle = 0.0, max_energy = 1.0):
	...
	if directional_light:
		...
		directional_light.light_energy = max_energy * (1.0 - clamp(abs(hours - 12.0) / 6.0, 0.0, 1.0))
